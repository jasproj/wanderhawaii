...
async function initWeather() {
    ...
    // Hawaii weather data - using approximate averages since we can't make external API calls
    // In production, replace with real API like OpenWeather
    const hawaiiWeather = {
        islands: [
            { name: 'Honolulu', temp: 82, icon: '☀️' },
            { name: 'Maui', temp: 80, icon: '⛅' },
            { name: 'Kona', temp: 84, icon: '☀️' },
            { name: 'Kauai', temp: 79, icon: '🌤️' }
        ]
    };
    
    // Pick a random island to display
    const island = hawaiiWeather.islands[Math.floor(Math.random() * hawaiiWeather.islands.length)];
    
    // Add slight temperature variation (+/- 3 degrees)
    const tempVariation = Math.floor(Math.random() * 7) - 3;
    const displayTemp = island.temp + tempVariation;
    
    weatherEl.innerHTML = `
        <span class="weather-icon">${island.icon}</span>
        <span class="weather-temp">${displayTemp}°F</span>
        <span class="weather-location">${island.name}</span>
    `;
    
    // Rotate through islands every 30 seconds
    setInterval(() => {
        const newIsland = hawaiiWeather.islands[Math.floor(Math.random() * hawaiiWeather.islands.length)];
        const newTempVariation = Math.floor(Math.random() * 7) - 3;
        const newDisplayTemp = newIsland.temp + newTempVariation;
        
        weatherEl.style.opacity = '0';
        setTimeout(() => {
            weatherEl.innerHTML = `
                <span class="weather-icon">${newIsland.icon}</span>
                <span class="weather-temp">${newDisplayTemp}°F</span>
                <span class="weather-location">${newIsland.name}</span>
            `;
