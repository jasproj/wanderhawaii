#!/usr/bin/env node
/**
 * wanderhawaii v5.2 live applicator.
 *
 * Reads scripts-staging/v52-wh-dryrun-raw.json (the validated 1,249-tour
 ...
 *
 * Backup: writes the pre-mutation file to /tmp/tours-data-wh.<ISO>.bak
 *         before overwriting.
 */
...
(function main() {
  if (!fs.existsSync(RAW_FILE)) {
    ...
    console.error('  node scripts-staging/run-v52-wh-dryrun.js --dry-run-only');
    process.exit(2);
  }
  ...
  // Card-flip count using the same logic as wanderhawaii app.js:92-96
  ...
  const backupPath = `/tmp/tours-data-wh.${ts}.bak`;
  fs.writeFileSync(backupPath, dataRaw);
  console.log(`\nBackup: ${backupPath}`);

  const out = data.tours ? { ...data, tours } : tours;
  // wanderhawaii stores literal UTF-8 chars (no \uXXXX escapes), so JSON.stringify default is correct.
...
function countCheckAvailability(tours) {
  // wanderhawaii app.js:92-96 formatPrice:
