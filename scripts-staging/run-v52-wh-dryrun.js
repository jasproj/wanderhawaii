#!/usr/bin/env node
/**
 * wanderhawaii v5.2 dry-run price extraction.
 *
 * Pulls every tour with `price === null` from tours-data.json (~1,249),
 ...
 *
 * Output:
 *   - scripts-staging/v52-wh-dryrun-raw.json (per-tour records)
 *   - scripts-staging/v52-wh-dryrun.md (human-readable report)
 *
 * Flag: --dry-run-only  (required; refuses to run otherwise)
 *
 * Usage:
 *   node scripts-staging/run-v52-wh-dryrun.js --dry-run-only
 *   node scripts-staging/run-v52-wh-dryrun.js --dry-run-only --limit 20  # quick smoke
 */
...
const RAW_FILE = 'scripts-staging/v52-wh-dryrun-raw.json';
const REPORT_FILE = 'scripts-staging/v52-wh-dryrun.md';
...
(async () => {
  ...
  console.log(`WH null-price tours to evaluate: ${targets.length}`);
  ...
  const lines = [];
  lines.push('# wanderhawaii v5.2 Dry-Run Report — null-price tour re-extraction');
  lines.push('');
  lines.push(`**Generated:** ${new Date().toISOString()}`);
  lines.push(`**Branch:** \`feat/wh-v52-price-extraction\``);
  lines.push(`**Mode:** \`--dry-run-only\` (no writes to tours-data.json)`);
  lines.push('');
  lines.push('## 1. Inputs');
  lines.push('');
  lines.push(`- wanderhawaii total tours: 3142`);
